...
// Strip trailing filename so the launcher works when served from nested paths.
...
function renderLibrary(list) {
    ...
    if (!select) return;
    const data = list || libraryFiltered;
    select.innerHTML = "";
    const placeholder = document.createElement("option");
    placeholder.value = "";
    placeholder.textContent = data && data.length ? `Chon game (${data.length})` : "Khong co game";
...
function attachLibraryHandlers() {
    ...
    if (search) {
        search.addEventListener("input", () => {
            ...
            if (!q) {
                libraryFiltered = libraryEntries;
            } else {
                libraryFiltered = libraryEntries.filter((it) => {
                    return (
            ...
            renderLibrary();
            setLibraryStatus(
                libraryFiltered && libraryFiltered.length
                    ? `Tim thay ${libraryFiltered.length} game phu hop.`
                    : "Khong tim thay game phu hop."
            );
        });
    }
    const reloadBtn = document.getElementById("library-reload");
    if (reloadBtn) {
        reloadBtn.onclick = () => initLibrary(true);
    }

    const installBtn = document.getElementById("library-install");
    const select = document.getElementById("library-select");
    if (installBtn && select) {
        installBtn.onclick = () => {
            const url = select.value;
            if (!url) {
                setLibraryStatus("Hay chon mot game tu danh sach.");
                return;
            }
            const item = (libraryFiltered || []).find((it) => it.url === url) || { url, name: select.options[select.selectedIndex]?.textContent };
            installLibraryItem(item, installBtn);
        };
    }
}

async function initLibrary(force) {
    if (libraryLoading) return;
    libraryLoading = true;
    setLibraryStatus("Dang tai thu vien...");
    const select = document.getElementById("library-select");
    if (select) select.innerHTML = "";
    try {
        const res = await fetch("j2me-library.json" + (force ? `?t=${Date.now()}` : ""));
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (!Array.isArray(data)) {
            throw new Error("Du lieu thu vien khong hop le");
        }
        libraryEntries = data;
        libraryFiltered = data;
        renderLibrary();
        setLibraryStatus(`Co ${data.length} game trong thu vien. Bam 'Cai' de nap truc tiep.`);
    } catch (e) {
        console.error("Load library failed", e);
        setLibraryStatus("Khong tai duoc thu vien rom/j2me. Kiem tra file j2me-library.json.");
    } finally {
        libraryLoading = false;
    }
}

async function installLibraryItem(item, btn) {
    if (!item || !item.url) return;
    if (libraryLoading) return;
    const prevText = btn ? btn.textContent : "";
    if (btn) btn.disabled = true;
    try {
        setLibraryStatus(`Dang tai ${item.name || item.url}...`);
        setupAddMode();
        ...
        await processGameFile(buf, item.name || item.url.split("/").pop());
        setLibraryStatus(`Da nap ${item.name || item.url}. Dieu chinh setting neu can va Save.`);
    } catch (e) {
        console.error("Install library item failed", e);
        setLibraryStatus(`Loi tai ${item.name || item.url}: ${e.message}`);
        if (btn) btn.disabled = false;
    } finally {
        if (btn) btn.textContent = prevText || "Cai";
    }
}
